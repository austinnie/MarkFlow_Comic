# skills/image/style_transfer/skill.py
"""
风格转换 Skill - 将图片转换为不同艺术风格（水彩、油画、动漫等）
...
STYLE_MAP = {
    "watercolor": {
        "prompt": "watercolor painting, soft colors, flowing, artistic, beautiful, masterpiece, high quality",
        "negative": "ugly, deformed, oil painting, realistic, photorealistic, blurry, low quality"
    },
    "oil_painting": {
        "prompt": "oil painting, canvas texture, rich colors, artistic, beautiful, masterpiece, high quality",
        "negative": "ugly, deformed, watercolor, realistic, photorealistic, blurry, low quality"
    },
    "anime": {
        "prompt": "anime style, vibrant colors, cel shading, beautiful, detailed, masterpiece, high quality",
        "negative": "ugly, deformed, realistic, photorealistic, watercolor, oil, blurry, low quality"
    },
    "sketch": {
        "prompt": "pencil sketch, graphite, hand-drawn, artistic, beautiful, masterpiece, high quality",
        "negative": "ugly, deformed, color, realistic, photorealistic, blurry, low quality"
    },
    "realistic": {
        "prompt": "realistic, photorealistic, detailed, natural, beautiful, masterpiece, high quality",
        "negative": "ugly, deformed, cartoon, anime, sketch, watercolor, oil, blurry, low quality"
    },
    "cinematic": {
        "prompt": "cinematic, movie scene, dramatic lighting, beautiful, masterpiece, high quality",
        "negative": "ugly, deformed, cartoon, anime, sketch, watercolor, oil, blurry, low quality"
    }
}
...
class StyleTransfer:
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # ==================== 获取风格 ====================
            style = kwargs.get('style', self.config.get('default_style', 'watercolor'))
            ...
            if output_path is None:
                ...
                output_path = str(self.output_dir / f"{Path(abs_image_path).stem}_style_{style}_{timestamp}.png")

            # ==================== 调用底层 ControlNet 引擎 ====================
            if self._controlnet_engine is None:
                return {"status": "error", "error": "ControlNet 引擎不可用"}

            logger.info(f"目标风格: {style}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style", "-s", default="watercolor",
                        choices=list(STYLE_MAP.keys()), help="目标艺术风格")
