...
"""
遘ｻ髯､迚ｩ菴・Skill - 莉主崟迚・ｸｭ遘ｻ髯､謖・ｮ夂黄菴・
鮟倩ｮ､菴ｿ逕ｨ謇句勘驕ｮ鄂ｩ・瑚凶驕ｮ鄂ｩ逕滓・謌仙粥・悟・螟咲畑騾夂畑 ControlNet 蠑墓梼蟷ｶ驟榊粋 Inpaint 蜈懷ｺ・
"""
...
class RemoveObject:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"RemoveObject v{self.version} 蛻晏ｧ句喧螳梧・")
        logger.info(f"  隶ｾ螟・ {self.device}")
        logger.info(f"  蠎募ｱょｼ墓梼: {'笨・蜿ｯ逕ｨ' if self.controlnet_engine else '笶・荳榊庄逕ｨ'}")
    ...
    def _generate_manual_mask(self, image: Image.Image) -> Image.Image:
        ...
        print("\n" + "=" * 50)
        print("謇句勘扈伜宛隕∫ｧｻ髯､逧・黄菴・)
        print("=" * 50)
        print("  謖我ｽ城ｼ譬・ｷｦ髞ｮ扈伜宛隕∫ｧｻ髯､逧・玄蝓滂ｼ育區濶ｲ・・)
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # ==================== 隨ｬ荳豁･・夂函謌宣・鄂ｩ ====================
            # 螯よ棡菴蟆・擂諠ｳ隕∬・蜉ｨ蛹厄ｼ瑚ｿ咎㈹蜿ｯ莉･謗･蜈･ YOLO/SAM縲・
            # 逶ｮ蜑堺ｸｺ莠・ｨｳ螳夲ｼ檎峩謗･隹・畑謇句勘扈伜宛・亥庄莉･莨 skip_manual=True 霍ｳ霑・ｼ・
            if not kwargs.get('skip_manual', False):
                object_mask = self._generate_manual_mask(image)
            else:
                # 螯よ棡霍ｳ霑・焔蜉ｨ・檎函謌蝉ｸ荳ｪ蜈ｨ鮟醍噪驕ｮ鄂ｩ・育ｭ牙酔莠主・螻驥咲ｻ假ｼ・
            ...
            # 螯よ棡蠎募ｱょｼ墓梼蜿ｯ逕ｨ・御ｸ皮畑謌ｷ蜈∬ｮｸ・瑚ｰ・畑蠑墓梼逕滓・
            if self.controlnet_engine is not None:
                logger.info("  櫨 菴ｿ逕ｨ騾夂畑 ControlNet 蠑墓梼霑幄｡檎ｧｻ髯､・井ｿ晄戟蜴滓怏扈捺桷・・..")
                result = self.controlnet_engine.execute(
                    input_image_path=str(abs_image_path),
                    prompt=prompt,
                    negative_prompt=negative_prompt,
                    preprocessor_type="HED",      # 謠仙叙霓ｯ霎ｹ郛假ｼ御ｿ晉蕗閭梧勹
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="遘ｻ髯､迚ｩ菴灘ｷ･蜈ｷ v2.0")
