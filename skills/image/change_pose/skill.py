# skills/image/change_pose/skill.py
"""
...
class ChangePose:
    ...
    @property
    def controlnet_engine(self):
        """延迟加载 ControlNet 引擎"""
        if self._controlnet_engine is None:
            if not CONTROLNET_ENGINE_AVAILABLE:
                logger.warning("ControlNet 引擎不可用")
                return None
            try:
                from skills.image.controlnet_img2img.skill import ControlnetImg2Img
                ...
                self._controlnet_engine = None
        return self._controlnet_engine
