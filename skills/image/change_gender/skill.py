...
"""
諤ｧ蛻ｫ霓ｬ謐｢ Skill - 蟆・ｺｺ迚ｩ莉守塙諤ｧ霓ｬ荳ｺ螂ｳ諤ｧ謌門渚荵・
螟咲畑騾夂畑 ControlNet 蠑墓梼・・penPose 髞∝ｧｿ諤・ｼ碁ｫ伜ｹ・ｺｦ驥咲ｻ俶困鬪ｨ逶ｸ/蜿大梛/譖ｲ郤ｿ・・
"""
...
class ChangeGender:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"ChangeGender v{self.version} 蛻晏ｧ句喧螳梧・")
    ...
    def _setup_config(self):
        defaults = {
            'default_steps': 35,
            'default_strength': 0.75,  # 諤ｧ蛻ｫ霓ｬ謐｢髴隕∵隼蜿倬ｪｨ逶ｸ蜥悟書髟ｿ
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if direction not in GENDER_PROMPTS:
                return {"status": "error", "error": f"譛ｪ遏･譁ｹ蜷・ {direction}・悟庄逕ｨ: {list(GENDER_PROMPTS.keys())}"}
            ...
            logger.info(f"諤ｧ蛻ｫ霓ｬ謐｢: {direction.replace('_', ' -> ')}")
            logger.info(f"謠千､ｺ隸・ {prompt[:80]}...")

            # 菴ｿ逕ｨ OpenPose 髞∵ｭｻ蜈ｨ霄ｫ蟋ｿ諤・
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="諤ｧ蛻ｫ霓ｬ謐｢蟾･蜈ｷ v2.0")
    ...
    parser.add_argument("--direction", "-d", default="male_to_female",
                        choices=list(GENDER_PROMPTS.keys()), help="霓ｬ謐｢譁ｹ蜷・)
